...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initial setup for Hall of Fame
    if (hofContent && hofContent.classList.contains('hof-content-collapsed')) {
        fluidSimulation = createFluidSimulation(hallOfFameSection, hofContent);
    }

    // Initial setup for About section
    if (aboutContent && aboutContent.classList.contains('hof-content-collapsed')) {
        aboutFluidSimulation = createFluidSimulation(aboutSection, aboutContent);
    }

    // Add observer to watch for class changes in Hall of Fame
    const hofObserver = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            if (mutation.attributeName === 'class') {
                fluidSimulation = handleToggle(hofContent, fluidSimulation, hallOfFameSection);
            }
        });
    });

    if (hofContent) {
        hofObserver.observe(hofContent, { attributes: true });
    }
    ...
    const aboutObserver = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            if (mutation.attributeName === 'class') {
                aboutFluidSimulation = handleToggle(aboutContent, aboutFluidSimulation, aboutSection);
            }
        });
    });
